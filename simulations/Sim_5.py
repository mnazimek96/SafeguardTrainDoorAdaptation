    ...
    def simulate(self, cycles, window):
        ...
        def update(i):
            ...
            if i < 200:
                ...
                self.adapted_close = self.adapt(self.adapted_close, self.new_c)
                self.new = self.adapt(self.new, self.modified_y_open)
                self.new_c = self.adapt(self.new_c, self.modified_y_close)
            else:
                self.adapted_open = self.adapt(self.adapted_open, self.new)
                self.adapted_close = self.adapt(self.adapted_close, self.new_c)
